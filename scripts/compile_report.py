...
def _render_template(template_path, context):
    ...
    Autoescape is disabled because context values may contain trusted HTML
    fragments (table rows, embedded images) generated by this script.
    """
    try:
...
def build_html_context(report_data):
    ...
    for sample, data in report_data.get('per_sample', {}).items():
        ...
        for metric_name, getter in qc_metrics_order:
            val = getter(cs, rc, ic)
            qc_rows.append(f'<tr><td>{sample}</td><td>{metric_name}</td><td>{val}</td></tr>')
    ctx['qc_table_rows'] = ''.join(qc_rows) if qc_rows else (
        '<tr><td colspan="3"><em>No QC data available</em></td></tr>'
    )
